  ...
  describe("buildAuthUrl", () => {
    describe("Success", () => {
      it("without params", async () => {
        ...
        const state = await authClient.getLoginStateByAuthCode(
          code,
          redirectUri
        );
        console.log(state);

        const refreshLoginState = await authClient.refreshLoginState(
          state.refreshToken || ""
        );
        ...
        const parseAccessToken = await authClient.parseAccessToken(
          state.accessToken
        );
