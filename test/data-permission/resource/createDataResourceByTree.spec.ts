...
describe('createDataSourceByTree', () => {

  const createDataResourceDto = {
    namespaceCode : 'test',
    resourceName : 'testResource',
    resourceCode : 'testResource',
    struct : [{ code:'test', name:'test1' }],
    actions : ['read', 'get'],
  ...
  describe('Success', () => {
    it('create dataSource by tree successfully', async () => {
      const { statusCode, data, message} = await managementClient.createDataResourceByTree(createDataResourceDto);
  ...
  describe('Fail', () => {
    it('namespaceCode can not be empty', async () => {
      ...
      const { statusCode, message } = await managementClient.createDataResourceByTree(createDataResourceDto);
  ...
  describe('Fail', () => {
    it('struct can not be empty', async () => {
      ...
      expect(statusCode).toEqual(400);
      expect(message).toEqual('struct must contain at least 1 elements');
    });
  });
